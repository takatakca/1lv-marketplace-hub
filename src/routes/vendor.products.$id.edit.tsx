...
function Page() {
  const { id } = Route.useParams(); const existing = products.find(p => p.id === id);
  ...
  return (
    <div className="max-w-4xl">
      ...
      <form onSubmit={(e) => { e.preventDefault(); toast.success("Product updated"); nav({ to: "/vendor/products" }); }} className="space-y-5 rounded-xl border border-border bg-card p-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button type="button" onClick={() => toast.message("Product archived (demo)")} className="rounded-md border border-destructive/40 bg-destructive/5 px-4 py-2 text-sm font-semibold text-destructive">Archive</button>
        </div>
      </form>
    </div>
  );
